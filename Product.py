...
class ProductClass:
    def __init__(self, root):
        ...
        # ==เพิ่มเติม== #
        cmb_search = ttk.Combobox(SearchFrame, textvariable=self.var_searchby_id, values=("เลือก", "eid", "name"),
        ...
        txt_productid = Entry(self.root, textvariable=self.var_eid_id, font=("th sarabun psk", 15), bg="lightyellow").place(x=150, y=150, width=180)
        ...
        self.ProductTable = ttk.Treeview(emp_frame, columns=("eid", "name", "quantity", "price", "status"),
        ...
        self.ProductTable.heading("eid", text="รหัสสินค้า")
    ...
    def add(self):
        ...
        cur = con.cursor()
        try:
            if self.var_eid_id.get() == "":  # Check for product ID
                messagebox.showerror("Error", "จำเป็นต้องมีรหัสสินค้า", parent=self.root)
            else:
                cur.execute("SELECT * FROM product WHERE eid=?", (self.var_eid_id.get(),))
                row = cur.fetchone()
                if row is not None:
                    messagebox.showerror("Error", "มีรายการสินค้านี้แล้ว", parent=self.root)
                else:
                    cur.execute("INSERT INTO product (eid, name, quantity, price, status) VALUES(?,?,?,?,?)", (
                        self.var_eid_id.get(),
    ...
    def update(self):
        ...
        try:
            if self.var_eid_id.get() == "":
                ...
                cur.execute("UPDATE product SET name=?, quantity=?, price=?, status=? WHERE eid=?", (
                    self.var_name_id.get(),
                    self.var_quantity_id.get(),
                    self.var_price_id.get(),
                    self.var_status_id.get(),
    ...
    def delete(self):
        ...
        try:
            if self.var_eid_id.get() == "":
                ...
                cur.execute("DELETE FROM product WHERE eid=?", (self.var_eid_id.get(),))
                con.commit()
